...
from src.core.exceptions.database_exceptions import (PostAlreadyExistsException,
                                                 PostNotFoundException)
from src.infrastructure.sqlite.models.posts import Post as PostModel
from src.schemas.posts import PostUpdateData
...
class PostRepository:
    ...
    def create(self, session: Session, post_data: dict) -> PostModel:
        ...
        try:
            ...
            session.rollback()
            raise PostAlreadyExistsException(title=post_data.get("title"))
        except SQLAlchemyError:
            session.rollback()
            raise PostAlreadyExistsException(title=post_data.get("title"))
