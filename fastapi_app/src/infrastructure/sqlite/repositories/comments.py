...
class CommentRepository:
    ...
    def get_by_post(self, session: Session, post_id: int, only_published: bool = True) -> List[CommentModel]:
        query = select(self._model).where(self._model.post_id == post_id)

        if only_published:
            query = query.where(self._model.is_published == True)

        query = query.order_by(self._model.created_at)
        return list(session.scalars(query))
    
    def create(self, session: Session, comment: CommentCreate, author_id: int) -> CommentModel:
    ...
    def update(self, session: Session, comment_id: int, user_id: int, comment_data: CommentUpdateData) -> CommentModel:
        comment = self.get_by_id(session, comment_id)

        if comment.author_id != user_id:
            raise PermissionError('You do not have permission to edit this comment')
        
        update_values = {k: v for k, v in comment_data.model_dump().items() if v is not None}
